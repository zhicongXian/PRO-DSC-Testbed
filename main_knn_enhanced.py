...
model = PRO_DSC_Transformer(input_dim=768, hidden_dim=args.hidden_dim, z_dim=args.z_dim).to(device) # input_dim=768
...
with tqdm(total=args.epo) as progress_bar:
    for epoch in range(args.epo):
        ...
        for step, (x, y, id_num) in enumerate(train_loader):
            x, y, id_num = x.float().to(device), y.to(device), id_num.float().to(device)
