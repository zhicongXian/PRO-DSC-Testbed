...
constant_derived_in_cifar10 = math.sqrt(-1/math.log(0.8007598568296741))
                ...
                with autocast(enabled=True):
                    ...
                    if epoch == total_wamup_epochs:
                        with torch.no_grad():
                            ...
                            # P = np.array(P)
                            B = c_matrix # P / (-np.diag(P) + 1e-7 * np.eye(G.shape[0]))
                            B[diagIndices] = 0
                            ...
                            try:
                                ...
                                gamma_estimated = args.beta *  math.sqrt(soft_rank_global)/2 * constant_derived_in_cifar10 / math.sqrt(abs(1/math.log(density_score)) )# 1 / lambda_hat
                                ...
                                try:
                                    ...
                                    soft_rank_global = frobi ** 2 / (l2_norm_b ** 2 + 1e-16)
                                    # print("soft_rank_global", soft_rank_global)
                                    gamma_estimated = args.beta * math.sqrt(soft_rank_global)/2* constant_derived_in_cifar10 / math.sqrt(abs(1/math.log(density_score)))
                                except Exception as e:
                                    print(e)
                                gamma_estimated = gamma_previous
                            print("current estimated gamma is ", gamma_estimated)
