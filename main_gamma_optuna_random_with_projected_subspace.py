...
from utils import *
from metrics.clustering import (spectral_clustering_metrics, spectral_clustering_metrics_with_projected_subspace_distance,
...
with open(os.path.join('configs','{}.yaml'.format(args.data.lower())), 'r', encoding='utf-8') as file:
    ...
    for key, value in yaml_data.items():
        if key == "experiment_name" or key =="seeds":
            continue
...
def objective( trial : optuna.trial.Trial):
        ...
        for epoch in range(config['epo']):
            ...
            if (epoch + 1) % config['validate_every'] == 0 or (epoch + 1) == config['epo'] or epoch ==warmup_epochs or early_stop:
                ...
                with torch.no_grad():
                    ...
                    acc_lst, nmi_lst, pred_lst, ari_lst, sde_lst, si_lst, nc_list, si_subspace_lst = spectral_clustering_metrics_with_projected_subspace_distance(x_np, self_coeff.detach().cpu().numpy(),args.n_clusters, y_np,
                                                                                                                                     seeds=[config['seed']])
                    ...
                    si_score_subspace = np.mean(np.asarray(si_subspace_lst))
                    ...
                    print(
                        'Logits head mean acc: {} max acc: {} mean nmi: {} max nmi: {}, mean ari: {} max ari: {}, mean sdi: {}, max sdi: {}, mean point-to-point  epoch {}\n'.format(
                                np.mean(acc_lst), np.max(acc_lst),
                                np.mean(nmi_lst), np.max(nmi_lst), np.mean(ari_lst), np.max(ari_lst), np.mean(sde_lst), np.max(sde_lst), epoch))
                    ...
                    result_df = pd.concat([result_df, pd.DataFrame.from_records(
                        [{'seq_name': args.data.lower(), 'seed': config['seed'], 'epoch': epoch, 'gamma_default': config['gamma'],
                          ...
                          'p2p_subspace_based': np.mean(si_subspace_lst),
                          'time': t_end - t_begin
                          }])]
                        )
    ...
    return -si_score_subspace #-si_score
