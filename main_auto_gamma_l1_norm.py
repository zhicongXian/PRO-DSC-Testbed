...
with open(os.path.join('configs','{}.yaml'.format(args.data.lower())), 'r', encoding='utf-8') as file:
    ...
    for key, value in yaml_data.items():
        if (hasattr(args, "experiment_name")  and key== "experiment_name") or key=="seeds":
            continue
...
for seed in args.seeds:
    ...
    if args.data.lower() in ['cifar10', 'cifar100', 'cifar20']:
        ...
        with open(previous_path + '/{}/{}_scattering_train_data.pkl'.format(args.data.upper(), args.data.upper()),
                  'rb') as f:
            train_samples = pickle.load(f)
        with open(previous_path + '/{}/{}_scattering_train_label.pkl'.format(args.data.upper(), args.data.upper()),
                  'rb') as f:
            train_labels = pickle.load(f)
        with open(previous_path + '/{}/{}_scattering_test_data.pkl'.format(args.data.upper(), args.data.upper()),
                  'rb') as f:
            test_samples = pickle.load(f)
        with open(previous_path + '/{}/{}_scattering_test_label.pkl'.format(args.data.upper(), args.data.upper()),
                  'rb') as f:
        ...
        for epoch in range(args.epo):
            ...
            for step, (x, y, id_num) in enumerate(train_loader):
                ...
                with autocast(enabled=True,device_type="cuda"):
                    ...
                    if epoch>= total_wamup_steps - 1: # run on every steps and warmup_step <= total_wamup_steps + nb_steps_per_epoch   no initial pretraining is used:
                        with torch.no_grad():
                            block = z.detach().clone().double()

                            # estimating soft rank ####################
                            # G =  block @ block.T
