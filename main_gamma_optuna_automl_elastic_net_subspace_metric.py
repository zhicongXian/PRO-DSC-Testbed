...
def objective( trial : optuna.trial.Trial):
        ...
        for epoch in range(config['epo']):
            ...
            for step, (x, y) in enumerate(train_loader):
                ...
                with autocast(enabled=True):
                    ...
                    if epoch <= warmup_epochs:
                        ...
                        loss_elastic_net = (config['lambda'] * torch.abs(self_coeff)).sum() + (
                                    (1.0 - config['lambda']) / 2.0 * torch.pow(self_coeff, 2)).sum()
                        loss_elastic_net = loss_elastic_net / config['bs']

                        loss = loss_tcr + config['gamma'] * loss_exp + config[
                            'beta'] * loss_elastic_net  # config['beta'] * loss_bl
            ...
            if (epoch + 1) % config['validate_every'] == 0 or (epoch + 1) == config['epo'] or epoch ==warmup_epochs or early_stop:
                ...
                with torch.no_grad():
                    ...
                    acc_lst, nmi_lst, pred_lst, ari_lst, sde_lst, si_lst, nc_list, si_subspace_lst = spectral_clustering_metrics_with_projected_subspace_distance(
                    ...
                    si_subspace_score = np.mean(np.asarray(si_subspace_lst))
                    ...
                    result_df = pd.concat([result_df, pd.DataFrame.from_records(
                        [{'seq_name': args.data.lower(), 'seed': config['seed'], 'epoch': epoch, 'gamma_default': config['gamma'],
                          ...
                          'subspace_score': si_subspace_score,
                          'time': t_end - t_begin
                          }])]
                        )
    ...
    return -si_subspace_score #-si_score
