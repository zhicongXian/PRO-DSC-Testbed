                ...
                if (epoch + 1) % config['validate_every'] == 0 or (epoch + 1) == config['epo'] or epoch ==warmup_epochs or early_stop:
                    ...
                    with torch.no_grad():
                        ...
                        result_df = pd.concat([result_df, pd.DataFrame.from_records(
                            [{'seq_name': seq_name, 'seed': config['seed'], 'epoch': epoch, 'gamma_default': config['gamma'],
                              'acc': np.mean(acc_lst),
                              ...
                              'p2p_subspace_based':si_subspace_score,
                              'time': t_end - t_begin
                              }])]
                            )
                        ...
                        if wandb.run:
                            wandb.log({
                                ...
                                'silhouette_score': si_score,
                                "gamma":config['gamma'],
                                'si_subspace_score': si_subspace_score,
                            })
